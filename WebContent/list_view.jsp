...
<c:choose>
	...
	<tr>
		...
		<td>
			...
			<c:set var="query" 
				value="articleId=${article.articleId}&p=${listModel.requestPage}"/>
	...
	<tr>
		...
		<c:if test="${beginPage > 2}">
			<a href="<c:url value="/articleList?p=${beginPage-1}"/>">이전</a>
		</c:if>
		<c:forEach var="pno" begin="${beginPage}" end="${endPage}">
		<a href="<c:url value="/articleList?p=${pno}" />">[${pno}]</a>
		</c:forEach>
		<c:if test="${endPage < listModel.totalPageCount}">
			<a href="<c:url value="/articleList?p=${endPage + 1}"/>">다음</a>
		</c:if>
		</td>
	</tr>
	</c:otherwise>
